...
export default async function EditPostPage({
  ...
  const saveData = async (data: FormData) => {
    "use server";
    await kv.hset("posts:1", {
      title: data.get("title"),
      content: data.get("content"),
    });
    revalidatePath("/posts/1/edit");
  };
  const saveDataAndRedirect = async (data: FormData) => {
    "use server";
    await kv.hset("posts:1", {
      title: data.get("title"),
      content: data.get("content"),
    ...
    <section className="text-gray-600 body-font relative">
      <div className="container px-5 py-24 mx-auto flex justify-center">
        <div className="lg:w-1/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col w-full mt-10 md:mt-0 relative z-10 shadow-md">
          ...
          <p className="leading-relaxed mb-5 text-gray-600">{post?.content}</p>
          <form action={saveData}>
            ...
            <div className="flex justify-between">
              ...
              <button
                formAction={saveDataAndRedirect}
